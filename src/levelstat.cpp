...
/***** global variables used for accessing the system clock (SDL-based) *****/

struct timeval current_time;

static void UpdateCurrentTime(void)
{
	uint64_t us = (uint64_t)SDL_GetTicks() * 1000ULL;
	current_time.tv_sec  = (int32)(us / 1000000ULL);
	current_time.tv_usec = (int32)(us % 1000000ULL);
}
...
/***********************************  Init3D0System  ************************************
   Platform initialization (SDL-based port).
*****************************************************************************************/
	...
	do
	{
		...
		if (enemies.seeker_list == (dude *) NULL)
		{
			...
			if (lives_per_level == 1)
				SetLevelFlagInStatusRecordFile(level,
				                               g_skill_level);
		}
		else
...
int main(int argc, char** argv)
{
	...
	for (i = 1; i <= 30; i++)
		SetLevelFlagInStatusRecordFile(i, INSANE);
	for (i = 31; i <= 95; i++)
		SetLevelFlagInStatusRecordFile(i, HARD);
//	SetLevelFlagInStatusRecordFile(96, EASY);
	for (i = 97; i <= 123; i++)
		SetLevelFlagInStatusRecordFile(i, HARD);
	SetLevelFlagInStatusRecordFile(124, EASY);
	for (i = 125; i <= 129; i++)
		SetLevelFlagInStatusRecordFile(i, HARD);
	SetLevelFlagInStatusRecordFile(130, EASY);
	SetLevelFlagInStatusRecordFile(131, HARD);
//	SetLevelFlagInStatusRecordFile(132, EASY);
	for (i = 133; i <= 141; i++)
		SetLevelFlagInStatusRecordFile(i, HARD);
	SetLevelFlagInStatusRecordFile(142, MEDIUM);
	SetLevelFlagInStatusRecordFile(143, MEDIUM);
	for (i = 144; i <= 147; i++)
		SetLevelFlagInStatusRecordFile(i, HARD);
	for (i = 148; i <= 150; i++)
		SetLevelFlagInStatusRecordFile(i, MEDIUM);
	exit(0);
#endif
