...
static  struct timeval  old_time;
	...
	do
	{
		SDL_Delay(1);  /* yield CPU briefly (replaces WaitVBL) */
		UpdateCurrentTime();
	}
...
{
	/* On the PC port, SDL provides timing. Just seed current_time. */
	g_clock_start_us = (uint64_t)SDL_GetTicks() * 1000ULL;
	UpdateCurrentTime();
	return(TRUE);
}

/********************************  ShutdownSystemClock  *********************************
   No-op on SDL — nothing to tear down.
*****************************************************************************************/

void ShutdownSystemClock()
{
	/* nothing to do on SDL */
}
...
{
	if (!OpenGraphics(&g_screen, 2))
	{
		printf("Init3D0System: Couldn't initialize graphics\n");
		return (FALSE);
	}
	
	/* OpenMathFolio — no-op on PC */
	
	/* InitEventUtility is mapped to InitInput in platform/input.h */
	...
	vbl = 0;  /* WaitVBL ioreq handle — unused on PC */
	return (TRUE);
}
